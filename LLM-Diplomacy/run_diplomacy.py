...
PHASES = ["CONVERSATION","ORDERS","RETREAT","BUILD"]
...
class ModelStateManager:
    ...
    def generate_prompt(self, game, country, phase, phase_index = "", conv_file = None):
        ...
        prompt = prompt.replace("[Insert current season, e.g., Spring 1901]", current_phase)
        prompt = prompt.replace("[Insert player’s country]", country)
        prompt = prompt.replace("[Insert brief summary of unit positions/territories]", 
                              self.get_brief_board_summary(game, country))

        scratch_content = self.get_scratchpad(country)
        scratch_section = f"<scratch>\n{scratch_content if scratch_content else 'No notes yet'}\n</scratch>"
        prompt = prompt.replace("[Insert existing scratch notes or leave blank]", scratch_section)
        
        phase_instructions = self.get_phase_instructions(phase, phase_index)
        prompt = prompt.replace("[SUMMARY OF PHASE AND INSTRUCTIONS]", phase_instructions)
        
        if phase.startswith("REPLY"):
            if os.path.exists(conv_file):
                with open(conv_file, 'r') as f:
                    current_conv = f.read()
                    prompt = prompt.replace("[OPTIONAL CURRENT CONVERSATION]", f"CURRENT CONVERSATION:\n{current_conv}")
            else:
                prompt = prompt.replace("[OPTIONAL CURRENT CONVERSATION]", "")
        else:
            prompt = prompt.replace("[OPTIONAL CURRENT CONVERSATION]", "")
        ...
        prompt = prompt.replace("(If applicable: Your available moves are: [List moves/rules here])", 
                            f"{moves_header}\n{available_moves}")
    ...
    def get_model_response(self, country: str, prompt: str) -> str:
        ...
        self._process_response(country, response_text)
        cleaned_response = self._clean_response(response_text)
        return cleaned_response

    def _process_response(self, country: str, response: str) -> None:
    ...
    def get_brief_board_summary(self, game, country):
        summary = []
        summary.append(f"Controlled Supply Centers: {', '.join(sorted(game.get_centers(country)))}")
        neighbor_states = set()
        for unit in game.get_units(country):
            loc = unit[2:]
            neighbors = game.map.abut_list(loc)
            neighbor_states.update(neighbors)
        summary.append("Adjacent Regions:")
        for region in sorted(neighbor_states):
            details = []
            if region in game.map.scs:
                details.append("(Supply Center)")
            for power, units in game.get_units().items():
                if any(unit.endswith(region) for unit in units):
                    details.append(f"{power} units")
            summary.append(f"- {region}: {', '.join(details)}")
        return "\n".join(summary)
    ...
    def get_phase_instructions(self, phase, phase_index):
        instructions = {
            "CONVERSATION": "CONVERSATION PHASE" + phase_index + ": Initiate one private conversation. Start message with recipient country name.",
            "REPLY": "REPLY PHASE " + phase_index + ": Continue conversation or end with <END>.",
            "ORDERS": "ORDERS PHASE: Submit your final orders using valid syntax from the list of availible orders. try to move at least one unit per turn.",
            "RETREAT": "RETREAT PHASE: Submit retreat orders or disband.",
            "BUILD": "BUILD PHASE: Submit build orders based on supply centers."
        }
    ...
    while not game.is_game_done:
        for phase in PHASES:
            if (phase == "CONVERSATION"):
                for i in [" 1 of 3", " 2 of 3"]: #, "2 of 3", "3 of 3" making it one round for testing
                    
                    for country in state_manager.models:
                        print(country, "CONVERSATION" + i)
                        ...
                        recipient = has_valid_recipient(response, state_manager)
                        if recipient:
                            process_conversation(game, country, recipient, response, state_manager)
                            #summarize_conversation(country, recipient, state_manager)

            elif phase == "ORDERS":
                for country in state_manager.models:
                    prompt = state_manager.generate_prompt(game, country, phase)
                    state_manager._save_full_response(country, "DEBUG: THIS IS THE END OF ORDERS PHASE")
                    response = state_manager.get_model_response(country, prompt)
                    state_manager.process_orders(game, country, response)


            elif game.get_current_phase().endswith('R') and phase == "RETREAT":
                for country in state_manager.models:
                    prompt = state_manager.generate_prompt(game, country, phase)
                    response = state_manager.get_model_response(country, prompt)
                    state_manager.process_orders(game, country, response)

            elif game.get_current_phase().endswith('B') and phase == "BUILD":
                for country in state_manager.models:
                    prompt = state_manager.generate_prompt(game, country, phase)
                    response = state_manager.get_model_response(country, prompt)
                    state_manager.process_orders(game, country, response)
...
def has_valid_recipient(message, state_manager):
    lines = message.split('\n')
    for line in lines:
        line = line.strip()
        if line:
            recipient = line.split()[0]
            if any(recipient == power for power in state_manager.models):
                return recipient
    return None
            

def process_conversation(game, sending_country, reciving_country, message, state_manager):
    conv_file = f"conversation_{sending_country}_{reciving_country}.txt"
    with open(conv_file, 'a') as f:
        f.write(f"{game.get_current_phase()} : {sending_country} - MESSAGE: {message}\n")
